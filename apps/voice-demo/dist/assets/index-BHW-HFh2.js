...
`),u.scrollTop=u.scrollHeight}function Y(){let e=n.getAll();if(e.length===0){p.textContent=`—`;return}p.innerHTML=e.map(e=>{let t=e.status===`PASS`?`green`:e.status===`FAIL`?`red`:`orange`,n=new Date(e.timestamp).toLocaleString();return`<div style="margin:0.2rem 0">
        ...
        `}function Z(){let e=_.value===`mic`;v.style.display=e?`none`:`block`,y.style.display=e?`block`:`none`}_.addEventListener(`change`,Z),Z();function Q(){T.textContent=r.getState();let e=r.getProgress();E.textContent=`${Math.min(e.currentScenario,e.totalScenarios)} / ${e.totalScenarios}`,D.textContent=`${e.progressPercent}%`;let t=r.getState()===C.Paused,n=r.getState()===C.Finished,i=r.getState()===C.WaitingTester;P.toggleAttribute(`disabled`,t||n),F.toggleAttribute(`disabled`,t||n),I.toggleAttribute(`disabled`,t||n),L.toggleAttribute(`disabled`,t||n),R.toggleAttribute(`disabled`,!t),oe.style.display=i?`block`:`none`}function $(){if(U>=H.length){be();return}let e=H[U],t=i().language,n=re(e,t);ae.textContent=ie(t),k.textContent=n.promptText,A.textContent=n.expectedText,r.beginScenario(1),he(),Q()}function ge(){H=[...O],U=0,W=[],N.style.display=`none`,r.startSession(H.length),$()}async function _e(){b.textContent=`🎤 Listening — say the phrase now…`,await t.channel.stop(),await t.channel.start(),s.textContent=t.channel.getState(),await new Promise(e=>{K=t=>{G&&(G.trigger=t.type),e()}}),b.textContent=`🎤 Idle`,J(),r.waitForTester(),Q()}async function ve(){let e=H[U];G={trigger:e,recognized:null,heard:null,comment:``,repeated:0,skipped:!1},_.value===`mic`?await _e():(await t.channel.injectAction({type:e,payload:{}}),J(),r.waitForTester(),Q())}function ye(){G&&=(G.comment=q||j.value,W.push(G),null),r.finishScenario(),r.nextStep(),U++,$()}function be(){r.stop(),_.value===`mic`&&(t.channel.stop(),s.textContent=t.channel.getState()),k.textContent=`Все сценарии пройдены.`,A.textContent=``,Q();let e=W.length,a=W.filter(e=>e.recognized&&e.heard).length,o=W.filter(e=>e.recognized!==e.heard).length,c=W.reduce((e,t)=>e+t.repeated,0),l=W.filter(e=>e.skipped).length;N.style.display=`block`,ce.innerHTML=`
        ...
        `;let u=V??i();V=u;let f={totalScenarios:e,passed:a,failed:e-a,errors:[]},p=t.executionLog.getEntries(),m=S(u,z,f,p);m.ManualValidation={results:W,warnings:o,repeatedSteps:c,skippedSteps:l},m.Summary&&(m.Summary.manualWarnings=o,m.Summary.repeatedSteps=c,m.Summary.skippedSteps=l),B=m,n.add(m),Y(),X(m),d.textContent=JSON.stringify(m,null,2)}P.addEventListener(`click`,async()=>{r.getState()===C.WaitingTester?ye():await ve()}),F.addEventListener(`click`,async()=>{G&&G.repeated++,await ve()}),I.addEventListener(`click`,()=>{G?(G.skipped=!0,W.push(G),G=null):W.push({trigger:H[U],recognized:null,heard:null,comment:``,repeated:0,skipped:!0}),r.finishScenario(),r.skipStep(),U++,$()}),L.addEventListener(`click`,()=>{r.pause(),Q()}),R.addEventListener(`click`,()=>{r.resume(),Q()}),le.addEventListener(`click`,()=>{G&&(G.recognized=!0)}),ue.addEventListener(`click`,()=>{G&&(G.recognized=!1)}),de.addEventListener(`click`,()=>{G&&(G.heard=!0)}),fe.addEventListener(`click`,()=>{G&&(G.heard=!1)}),h.addEventListener(`change`,()=>{let e=h.value===`interactive`;w.style.display=e?`block`:`none`,g.style.display=e?`block`:`none`,e&&(z=new Date().toISOString(),t.executionLog.clear(),u.textContent=``,ge())}),t.channel.onAction=e=>{if(t.logger.logAction(e),J(),K){let t=K;K=null,t(e)}},t.channel.onEvent=e=>{t.logger.logEvent(e),J()},t.channel.onSpeak=e=>{t.logger.logSpeak(e),J()},e.querySelector(`#btn-connect`).addEventListener(`click`,async()=>{let e=i();V=e;let n=await t.backend.connect(`https://ibronevik.ru/taxi/c/gruzvill`,e.login,e.password);a.textContent=n.status===`connected`?`● Connected`:`✗ `+n.status,o.textContent=n.status===`connected`?`Ready`:`—`}),e.querySelector(`#btn-start`).addEventListener(`click`,async()=>{z=new Date().toISOString(),t.executionLog.clear(),u.textContent=``,await t.channel.start(),s.textContent=t.channel.getState()}),e.querySelector(`#btn-stop`).addEventListener(`click`,async()=>{await t.channel.stop(),s.textContent=t.channel.getState()}),e.querySelector(`#btn-inject`).addEventListener(`click`,async()=>{let e=m.value;await t.channel.injectAction({type:e,payload:{}}),J()}),e.querySelector(`#btn-run-all`).addEventListener(`click`,async()=>{let e=V??i();V=e;for(let e=0;e<O.length;e++)c.textContent=`Running scenario ${e+1} of ${O.length}`,await t.channel.injectAction({type:O[e],payload:{}}),await new Promise(e=>{setTimeout(()=>e(),700)}),J();c.textContent=`Done`;let r=t.registry.list().length||3,a={totalScenarios:r,passed:r,failed:0,errors:[]};l.innerHTML=`<span style="color:green;font-weight:bold">✅ PASS (${a.passed}/${a.totalScenarios})</span>`;let o=t.executionLog.getEntries(),s=S(e,z,a,o);B=s,n.add(s),Y(),X(s),d.textContent=JSON.stringify(s,null,2)}),e.querySelector(`#btn-download`).addEventListener(`click`,()=>{if(!B){alert(`Run All first!`);return}let e=V??i(),t=new Blob([JSON.stringify(B,null,2)],{type:`application/json`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.href=n,r.download=ee(e),r.click(),URL.revokeObjectURL(n)}),e.querySelector(`#btn-send`).addEventListener(`click`,async()=>{if(!B){alert(`Run All first!`);return}let e=`https://ibronevik.ru/taxi/c/gruzvill`,n=await t.backend.getEmailId(e);if(!n){alert(`❌ Send failed: no email id available`);return}let r=await t.backend.sendReport(e,B,n);alert(r?`✅ Report sent!`:`❌ Send failed!`)})}var A=b();k(document.querySelector(`#app`),A);